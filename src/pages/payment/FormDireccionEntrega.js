  ...
  return (
    <>
      ...
      <div className="col-lg-12">
        <div className="billing-select mb-20">
          ...
          <select name="idDepartamento" required={true} onChange={handleChange}>
            <option name="idDepartamento">
              {t("page_checkout.select_country")}
            </option>
            {country &&
              country.departamentos.map((depto, index) => (
                <option key={depto.id || `depto-${index}`} value={depto.id}>
                  {depto.Nombre}
                </option>
              ))}
          </select>
      ...
      {/* <div className="col-lg-6 col-md-6">
        <div className="billing-info mb-20">
          <label className={clsx(!errorsValidate.phone && "text-danger")}>
            {t("page_checkout.phone1")}
          </label>
          <input
            type="tel"
            name="telefono"
          ...
          {!errorsValidate.email && (
            <p className="text-danger mt-1">{"telefono inválido"}</p>
          )}{" "}
      ...
      <div className="col-lg-6 col-md-6">
        <div className="billing-info mb-20">
          <label
            htmlFor="email"
            className={clsx(!errorsValidate.email && "text-danger")}
          >
            {t("page_checkout.email_address")}
          </label>
          <input
            type="email"
            name="email"
            ...
            onChange={handleChange}
            className={clsx(
              !errorsValidate.email && "border-danger text-danger fw-bold"
            )}
          />
          {!errorsValidate.email && (
            <p className="text-danger mt-1">{"correo inválido"}</p>
          )}{" "}
